...
class GanttChartFromActivities extends StatefulWidget {
  final List<Activity> activities;

  const GanttChartFromActivities({
    super.key,
    this.activities = const [],
  });

  @override
  State<GanttChartFromActivities> createState() => _GanttChartFromActivitiesState();
}

class _GanttChartFromActivitiesState extends State<GanttChartFromActivities> {
  ...
  void loadActivities() async {
    ...
    if (widget.activities.isEmpty) {
      var result = await getAssociatedActivities(context, true);
      if (mounted) {
        setState(() {
      ...
      loadedActivities = widget.activities;
    }
  }

  List<GanttAbsoluteEvent> convertActivitiesToGanttEvents(
    List<Activity> activities,
  ) {
    List<GanttAbsoluteEvent> ganttEvents = [];
    for (int i = 0; i < activities.length; i++) {
      var ganttEvent = GanttAbsoluteEvent(
        displayName: formatStringByLength(activities[i].description, 21),
        startDate: activities[i].start_date,
        endDate: activities[i].deadline_date,
      );
            ...
            children: [
                ...
                child: Column(
                  children: [
                    ...
                    GanttChartView(
                      ...
                      stickyAreaEventBuilder: customStickyArea
                          ? (context, eventIndex, event, eventColor) => eventIndex == 0
                              ? Container(
                                  ...
                                  child: Center(
                                    child: Text(
                                      "${AppLocalizations.of(context)!.customWidget}: ${event.displayName}",
                                    ),
                                  ),
                                )
                      ...
                      events: convertActivitiesToGanttEvents(loadedActivities),
                    ),
                  ],
                ),
              ),
              Container(
                height: _containerHeight(
                  convertActivitiesToGanttEvents(loadedActivities).length,
                ),
        ...
        floatingActionButton: Row(
          children: [
            ...
            Row(
              ...
              children: [
                MegaObraDefaultText(
                  text: AppLocalizations.of(context)!.showActivities,
                  size: 19,
                ),
                const SizedBox(width: 8),
                MegaObraSwitch(
