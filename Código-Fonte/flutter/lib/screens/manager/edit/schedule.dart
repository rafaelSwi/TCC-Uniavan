...
class EditScheduleScreen extends StatefulWidget {
  ...
  EditScheduleScreen({super.key, required this.schedule, required this.userAmount});
...
class _EditScheduleScreenState extends State<EditScheduleScreen> {
  ...
  void tryToEditSchedule(
    BuildContext context,
    ScheduleCreate newSchedule,
  ) async {
    try {
      await editSchedule(context, widget.schedule.id, newSchedule);
      if (mounted) {
        showAlertDialog(
          context: context,
          title: AppLocalizations.of(context)!.updateUserSchedule,
          text: AppLocalizations.of(context)!.requestToUpdateSchedule,
          onConfirm: () {},
          pop: true,
        );
      }
    } catch (e) {
      print("Error while trying to edit a schedule: $e");
      if (mounted) {
        showAlertDialog(
          context: context,
  ...
  void initState() {
    ...
    _nameController = TextEditingController(
      text: widget.schedule.schedule_name,
    );
    clock_in = TimeOfDay(
      hour: widget.schedule.clock_in.hour,
      minute: widget.schedule.clock_in.minute,
    );
    clock_out = TimeOfDay(
      hour: widget.schedule.clock_out.hour,
      minute: widget.schedule.clock_out.minute,
    );
    if (widget.schedule.break_in != null) {
      break_in = TimeOfDay(
        hour: widget.schedule.break_in!.hour,
        minute: widget.schedule.break_in!.minute,
      );
    }
    if (widget.schedule.break_out != null) {
      break_out = TimeOfDay(
        hour: widget.schedule.break_out!.hour,
        minute: widget.schedule.break_out!.minute,
      );
    }
    if ([widget.schedule.break_in, widget.schedule.break_out].contains(null)) {
      hasBreak = false;
    }
  }
      ...
      body: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 40),
            MegaObraDefaultText(
              text: widget.userAmount > 0 ? "${AppLocalizations.of(context)!.usersAffectedByChange}: ${widget.userAmount}." : "",
              size: 23,
            ),
            ...
            MegaObraButton(
              width: 300,
              height: 35,
              text: AppLocalizations.of(context)!.updateSchedule,
                    ...
                    if (mounted)
                      {
                        showQuestionAlertDialog(
                          ...
                          text: AppLocalizations.of(context)!.updateThisSchedule,
